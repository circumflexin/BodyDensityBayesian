...
sub_index = ave(is_sub, cumsum(!is_sub), FUN = cumsum)+1 #subglide counter, not very readable but trust me. 
...
modTab <- data.frame(
  modNum    = c(12),
  Vair.type = c("D"),  # Vair term
  CdAM.type = c("I"), # drag term
  BD.type   = c("I")) # body density
